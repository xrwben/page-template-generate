...
export default class PCScroller extends Component {
    ...
    _resetBox () {
        this.boxSize = {
            ...
            h: this.refsWrapper.current.scrollHeight - this.refsWrapper.current.clientHeight || 1,
            top: 0,
            ssTop: 0,
            ssHeight: 60 // 默认滚动条高度
        }
    ...
    _scroll (evt) {
        ...
        if (this.boxSize.top <= 0) {
            ...
            // * -10的原因是因为有些滚动条 滚到底部之后 还可以继续拉伸滚动 ? 具体导致原因不明
            this.boxSize.top = this.props.nodiff ? this.boxSize.h : this.boxSize.h - 10
        } else {
            evt.preventDefault()
            evt.stopPropagation()
