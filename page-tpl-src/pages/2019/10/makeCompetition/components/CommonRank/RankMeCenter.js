...
export default function RankMeCenter (props) {
    ...
    const showMask = meCenter.getin

    return (
        <div className="rank-center">
            <div className="prom-mask" style={ { display: showMask ? 'block' : 'none' } }>恭喜小主已晋级至擂主争霸赛</div>
            <div className="infor">
                <div className="headpic">
                    <img className="avatar" src={ meCenter.headPic } alt="主播头像" onClick={ () => goRoom(meCenter) } />
                    <span className="icon-s icon_live" style={ { display: meCenter.isPlaying ? 'block' : 'none' } }></span>
                </div>
